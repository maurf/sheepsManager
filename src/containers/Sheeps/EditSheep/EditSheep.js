...
/**
 * EditSheep Container 
 * Display the container to edit a sheep
 * 
 * TODO:
 */
const EditSheep = props => {
    
    const dispatch = useDispatch();
    const sheepSavedSuccess = useSelector( state => state.editSheepReducer.sheepSavedSuccess );  // Handles the success modal 
    const fetchedSheep = useSelector( state => state.editSheepReducer.sheep ); // Fecth the selected sheep from the API
    const sheepsList = useSelector( state => state.sheepManager.sheeps ); // Gets all sheeps from the state
    ...
    const [reproductiveCycles, setReproductiveCycles] = useState([]); //Sheep Reproductive Cycles
     ...
     /**
     * startNewSheep  
     * Dispatch initSheep action and fecth the selected sheep
     */
    const initSheep = useCallback(() => dispatch(actions.initSheep(props.match.params.id)), [props.match.params.id]);
    ...
    useEffect(() => {
        setSheepFormData(fetchedSheep ? {...fetchedSheep } : initialState);
        setReproductiveCycles(fetchedSheep ? fetchedSheep.reproductiveCycles : []);
    }, [fetchedSheep]) 

    /**
     * onSaveSheep  
     * Dispatch edit sheep action
     */
    const onSaveSheep = () =>  {
        const newSheep = {
            ...sheepFormData,
        ...
        dispatch(actions.editSheep(props.match.params.id, newSheep));
    }
    ...
    const redirectHome = () => props.history.replace("/");
    ...
    if (sheepSavedSuccess) {
        modal = <Modal 
                    ...
                    btnText="Ver Todas" clicked={redirectHome} 
                    cancelClicked={redirectHome} btnColor="green"/> 
    }       
    ...
    return (
        <div className="container mx-auto max-w-5xl px-6 py-8 relative">
            ...
            {/* Form Start */}
            <h3 className="text-gray-700 text-3xl font-medium mb-4">Editar Oveja</h3>
